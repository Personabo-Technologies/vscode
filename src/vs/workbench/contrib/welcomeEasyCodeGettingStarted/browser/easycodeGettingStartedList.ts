...
type EasyCodeGettingStartedIndexListOptions<T> = {
	title: string;
	klass: string;
	limit: number;
...
export class EasyCodeGettingStartedIndexList<T extends { id: string; when?: ContextKeyExpression }> extends Disposable {
	...
	constructor(
		private options: EasyCodeGettingStartedIndexListOptions<T>
	) {
		super();
